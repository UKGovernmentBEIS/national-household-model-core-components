...
public interface IInternalDimension<T> extends IDimension<T> {
	...
	/**
	 * Used only when merging a branch (see {@link IBranch#merge(IBranch, uk.org.cse.nhm.simulator.state.IStateChangeSource)};
	 * this is often something like set(instance, branch.get(instance));.
	 * 
	 * For derived dimensions, this offers the opportunity to avoid recomputing values if it is worth doing.
	 * 
	 * @param instance
	 * @param branch
	 */
	 ...
	 * 
	 * @param forkingState the new state to which this branch will belong; if this dimension
	 * 			depends on the values of other dimensions you can access their values in the new
	 * 			branch through this state
	 * @param capacity TODO
	 * @return
	 */
