...
/**
 * This is the counterpart to the {@link RealTimeDimension}, which supports variation in the current date depending on
 * the foresight levels of the person asking. The way this works is:
 * 
 * - the date is changed (typically advanced) or not for the different foresight levels by something else, like a function
 *   which predicts the future in some way.
 * - when a function is being computed, it will ask the surreal time dimension for the date at the forecast level that is
 *   required to be able to predict that function.
 *   
 * So, if the context cannot forecast tariffs, the value in {@link #times} under {@link XForesightLevel#Tariffs} will not advance.
 * When a time function is computed in a tariff, it will probably experience that foresight level as the value stored in the {@link ILets}.
 * That value will be passed to {@link #get(ILets)} through to {@link #get(XForesightLevel)} below, and so the unchanging date
 * is what will always be returned.
 */
public class SurrealTimeDimension implements ITimeDimension, IInternalDimension<ITime>, ITime {
	...
	public DateTime get(final ILets lets) {
		// shortcut for empty
		if (lets == ILets.EMPTY) return get(XForesightLevel.Default);
	...
	public DateTime get(final Optional<XForesightLevel> foresight, final ILets lets) {
		if (foresight.isPresent()) return get(foresight.get());
		else return get(lets);
	}
