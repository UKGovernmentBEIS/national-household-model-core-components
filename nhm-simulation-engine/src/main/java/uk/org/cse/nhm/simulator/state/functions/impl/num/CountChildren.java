...
public class CountChildren extends AbstractNamed implements IComponentsFunction<Number> {
	private final IDimension<People> people;

	@Inject
	public CountChildren(final IDimension<People> people) {
		this.people = people;
	}
	
	@Override
	public Number compute(final IComponentsScope scope, final ILets lets) {
		return scope.get(people).getChildren();
	}
