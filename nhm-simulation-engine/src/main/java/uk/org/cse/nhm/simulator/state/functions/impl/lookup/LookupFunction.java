...
/**
 * This is a function which implements a quite-fast n-dimensional map lookup, allowing
 * for certain kinds of range query on number-valued parts of the key.
 * 
 * To see how it works, head down to {@link #compute(IComponentsScope)}, as the efficiency
 * stuff in the implementation is slightly hard reading from the top.
 * 
 * @author hinton
 *
 */
public class LookupFunction extends AbstractNamed implements IComponentsFunction<Number> {
	...
	/**
	 * The functions which make up the compound key. When computing a value,
	 * we evaluate each of these functions to produce a compound key; then
	 * we find the first entry which matches the key.
	 */
	...
	/**
	 * This is an efficiency hack; if populated with constant components functions
	 * (i.e. simple numbers) then we will lookup into this table rather than value functions.
	 * We mark the difference by putting Double.NaN in here if the value function
	 * is non-constant.
	 */
	 ...
	 * It is indexed by key function (the array index) and then by value function
	 * (the index into the bitset), so if stars[i].get(j) is true then we
	 * don't care what value key function i produces for the purposes of selecting
	 * value function j.
	 */
	private final BitSet[] stars;
	/**
	 * This is a bunch of range sets, indexed by key table; each range set can tell you
	 * whether a particular value function is acceptable for some value of a given key;
	 */
	...
	/**
	 * This is an array which takes us from key to map from value to entries which
	 * accept that value for that part of the key.
	 * 
	 * For example, if exactMatches[i].get("Widget").get(j) is true then value function
	 * j is acceptable when the key has "Widget" in element i.
	 */
	...
	private Class<?> getConvertedType(final Class<?> inputType) {
		if (inputType.isEnum()) return inputType;
		else if (Double.class.isAssignableFrom(inputType)) return Double.class;
		else if (Integer.class.isAssignableFrom(inputType)) return Integer.class;
		else if (Number.class.isAssignableFrom(inputType)) return Double.class;
		else if (Boolean.class.isAssignableFrom(inputType)) return Boolean.class;
	...
	private Object convertKeyValue(final Object compute, final int i) {
		...
		if (desiredType.isInstance(compute)) return compute;
		
		if (desiredType == String.class) return String.valueOf(compute);
	...
	/**
	 * This returns a bitmask for the entries in the table which can have
	 * the given object in the given key position.
	 * 
