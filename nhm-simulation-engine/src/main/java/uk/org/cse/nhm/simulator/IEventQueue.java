...
/**
 * The event queue at the heart of the simulator; this is a callback driven system, so you register a callback to be executed using
 * {@link #addEvent(ICallback)}, and then {@link ICallback#run()} will execute when we get to that point in time
 * @author hinton
 *
 */
public interface IEventQueue {
	...
	/**
	 * Run all the events at the front of the queue until the time of next event from {@link #getTimeOfNextEvent()} has changed,
	 * or there are no events left to run.
	 * @return TODO
	 * @throws NHMException
	 */
	...
	/**
	 * Equivalent to <code>while(!isEmpty()) despatch();</code> - run the simulation until there's nothing left to do.
	 * @throws NHMException 
	 */
