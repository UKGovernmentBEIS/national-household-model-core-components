...
/**
 * This measure DOES NOT install central wet heating itself. Instead, it is used to record that it has been installed as a side effect of another measure.
 * 
 * See {@link ITechnologyOperations} for the various parts that actually do this job.
 * 
 */
public class WetHeatingMeasure extends AbstractMeasure {
	...
	/**
	 * Applies the given heating measure, checks if central wet heating was installed and, if so, makes a subscope and writes down technology installation details and capex for that wet heating system.
	 * 
	...
	public boolean isInstalled(final IComponentsScope scope) {
		return scope.get(technology).getPrimarySpaceHeater()
				instanceof ICentralHeatingSystem;
	}
