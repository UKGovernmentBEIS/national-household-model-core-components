...
/**
 * Lookup K values for walls; this is based on the data in the CHM spreadsheet,
 * as RDSAP doesn't seem to say much about this.
 *
 * Because the CHM spreadsheet only handles some combinations of insulation, I
 * have made the following assumptions:
 *
 * @assumption For those wall types where only the uninsulated k value is
 * specified, insulation does not change the k value (so the uninsulated value
 * is used).
 * @assumption For those wall types where there are multiple insulation types
 * present, the minimum k value from the table is chosen.
 *
 * @author hinton
 * @since 1.0
 */
    ...
    public double getKValue(
        ...
        if (insulationTypes.isEmpty() || !insulated.containsKey(constructionType)) {
            ...
            for (final WallInsulationType wit : insulationTypes) {
                if (map.containsKey(wit)) {
                    minimum = Math.min(minimum, map.get(wit));
                }
            }
