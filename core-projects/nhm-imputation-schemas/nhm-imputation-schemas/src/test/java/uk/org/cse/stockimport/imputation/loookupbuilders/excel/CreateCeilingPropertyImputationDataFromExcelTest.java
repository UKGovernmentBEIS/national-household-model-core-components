...
public class CreateCeilingPropertyImputationDataFromExcelTest extends AbsImputationFromExcelTest {
	...
	public void UValuesForCeilingsAreAreReadCorrectlyFromExcelSheet() throws FileNotFoundException, IOException{
		TreeMap<Integer, Double> pitchedValues = imputationSchema.getCeilingUValueTables().getInsulatedPitchedUValues();
		...
		TreeMap<Integer, Double> thatchedValues = imputationSchema.getCeilingUValueTables().getInsulatedThatchedUValues();
		assertNotNull("Should import pitched roof u value table", pitchedValues);
		...
		double[][] unkownValues = imputationSchema.getCeilingUValueTables().getUnknownValuesByRoofTypeAndAgeBand();
		...
		double[][] unkownValuesRoomInRoof = imputationSchema.getCeilingUValueTables().getUnknownValuesByRoofTypeAndAgeBandWithRoomInRoof();
		...
		assertThat("roofKValue", imputationSchema.getCeilingUValueTables().getRoofKValue(), equalTo(9d));
	}
}
