...
/**
 * This device takes a batch table and returns a wider batch table with fewer rows and more columns.
 * 
 * It finds all the group columns which have 2 or more values. It removes these from the group columns, and uses them to create more output columns instead.
 * New output columns = old output columns * combinations of groups
 * The resulting table should have only 1 row per scenario run.  
 */
class WidenBatchTable {
	...
	/**
	 * Remakes the main data table.
	 * Removes the grouping columns which were found to have multiple values.
	 * Uses them to fill in the data for the output columns.
	...
	/**
	 * Quite often we will get reports where one or more of the grouping columns only ever takes one value.
	 * For example, all our reports might have the same date (the end of the scenario).
	 * We don't want to include these in the widening process since they'd only clutter up the column names.
	 */
