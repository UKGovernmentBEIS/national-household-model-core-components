package uk.org.cse.nhm.ehcs10.physical.impl;

import uk.org.cse.nhm.ehcs10.derived.types.Enum10;
import uk.org.cse.nhm.ehcs10.physical.RoofstruEntry;
import uk.org.cse.nhm.ehcs10.physical.types.Enum1691;
import uk.org.cse.nhm.ehcs10.physical.types.Enum1706;
import uk.org.cse.stockimport.spss.SurveyEntryImpl;

public class RoofstruEntryImpl extends SurveyEntryImpl implements RoofstruEntry {

    private Integer BACK_Leave;
    private Integer FRONT_ReplacementPeriod;
    private Integer FRONT_TenthsOfArea;
    private Integer BACK_Age;
    private Integer BACK_Strengthen;
    private Integer FRONT_Strengthen;
    private Integer FRONT_Age;
    private Integer BACK_ReplacementPeriod;
    private Integer BACK_Replace;
    private Integer FRONT_Leave;
    private Integer FRONT_Replace;
    private Integer BACK_TenthsOfArea;
    private Enum1691 FRONT_Faults;
    private Enum1691 BACK_Faults;
    private Enum10 FRONT_Urgent_;
    private Enum1706 TypeOfRoofStructure;
    private Enum10 BACK_Urgent_;

    public Integer getBACK_Leave() {
        return BACK_Leave;
    }

    public void setBACK_Leave(final Integer BACK_Leave) {
        this.BACK_Leave = BACK_Leave;
    }

    public Integer getFRONT_ReplacementPeriod() {
        return FRONT_ReplacementPeriod;
    }

    public void setFRONT_ReplacementPeriod(final Integer FRONT_ReplacementPeriod) {
        this.FRONT_ReplacementPeriod = FRONT_ReplacementPeriod;
    }

    public Integer getFRONT_TenthsOfArea() {
        return FRONT_TenthsOfArea;
    }

    public void setFRONT_TenthsOfArea(final Integer FRONT_TenthsOfArea) {
        this.FRONT_TenthsOfArea = FRONT_TenthsOfArea;
    }

    public Integer getBACK_Age() {
        return BACK_Age;
    }

    public void setBACK_Age(final Integer BACK_Age) {
        this.BACK_Age = BACK_Age;
    }

    public Integer getBACK_Strengthen() {
        return BACK_Strengthen;
    }

    public void setBACK_Strengthen(final Integer BACK_Strengthen) {
        this.BACK_Strengthen = BACK_Strengthen;
    }

    public Integer getFRONT_Strengthen() {
        return FRONT_Strengthen;
    }

    public void setFRONT_Strengthen(final Integer FRONT_Strengthen) {
        this.FRONT_Strengthen = FRONT_Strengthen;
    }

    public Integer getFRONT_Age() {
        return FRONT_Age;
    }

    public void setFRONT_Age(final Integer FRONT_Age) {
        this.FRONT_Age = FRONT_Age;
    }

    public Integer getBACK_ReplacementPeriod() {
        return BACK_ReplacementPeriod;
    }

    public void setBACK_ReplacementPeriod(final Integer BACK_ReplacementPeriod) {
        this.BACK_ReplacementPeriod = BACK_ReplacementPeriod;
    }

    public Integer getBACK_Replace() {
        return BACK_Replace;
    }

    public void setBACK_Replace(final Integer BACK_Replace) {
        this.BACK_Replace = BACK_Replace;
    }

    public Integer getFRONT_Leave() {
        return FRONT_Leave;
    }

    public void setFRONT_Leave(final Integer FRONT_Leave) {
        this.FRONT_Leave = FRONT_Leave;
    }

    public Integer getFRONT_Replace() {
        return FRONT_Replace;
    }

    public void setFRONT_Replace(final Integer FRONT_Replace) {
        this.FRONT_Replace = FRONT_Replace;
    }

    public Integer getBACK_TenthsOfArea() {
        return BACK_TenthsOfArea;
    }

    public void setBACK_TenthsOfArea(final Integer BACK_TenthsOfArea) {
        this.BACK_TenthsOfArea = BACK_TenthsOfArea;
    }

    public Enum1691 getFRONT_Faults() {
        return FRONT_Faults;
    }

    public void setFRONT_Faults(final Enum1691 FRONT_Faults) {
        this.FRONT_Faults = FRONT_Faults;
    }

    public Enum1691 getBACK_Faults() {
        return BACK_Faults;
    }

    public void setBACK_Faults(final Enum1691 BACK_Faults) {
        this.BACK_Faults = BACK_Faults;
    }

    public Enum10 getFRONT_Urgent_() {
        return FRONT_Urgent_;
    }

    public void setFRONT_Urgent_(final Enum10 FRONT_Urgent_) {
        this.FRONT_Urgent_ = FRONT_Urgent_;
    }

    public Enum1706 getTypeOfRoofStructure() {
        return TypeOfRoofStructure;
    }

    public void setTypeOfRoofStructure(final Enum1706 TypeOfRoofStructure) {
        this.TypeOfRoofStructure = TypeOfRoofStructure;
    }

    public Enum10 getBACK_Urgent_() {
        return BACK_Urgent_;
    }

    public void setBACK_Urgent_(final Enum10 BACK_Urgent_) {
        this.BACK_Urgent_ = BACK_Urgent_;
    }

}
