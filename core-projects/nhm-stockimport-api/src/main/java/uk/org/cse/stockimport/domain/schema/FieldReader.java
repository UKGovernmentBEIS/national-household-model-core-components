...
/**
 * Utility for reading/writing values from fields. This has probably been
 * written 9 million times before.
 *
 * @author hinton
 *
 * @param <T>
...
abstract class FieldReader<T> {
    ...
    public static <T> FieldReader<T> of(final TypeToken<T> token) {
        ...
        } else if (new TypeToken<List<? extends Object>>() {
        ...
        } else if (new TypeToken<Optional<? extends Object>>() {
        }.isAssignableFrom(token)) {
            try {
