...
/**
 * Build step which takes an {@link ISpaceHeatingDTO} and creates space heating
 * systems from it.
 *
 * @author hinton
 * @since 1.0
...
public class SpaceHeatingBuildStep implements ISurveyCaseBuildStep {
    ...
    private void build(final SurveyCase model, final IHouseCaseSources<IBasicDTO> dtoProvider, final ISpaceHeatingDTO dto) {
        ...
        if (system != null) {
            system.getControls().addAll(dto.getHeatingSystemControlTypes());
        }
    ...
    /**
     * A final fallback which will install an electric room heater if there are
     * no heating systems
     *
     * @param tech
     */
     ...
     *
     * If there is already a secondary heater which has the same fuel type, copy
     * the responsiveness & efficiency out of secondaryHeater.
     *
     * @param aacode
     * @param tech
     ...
     *
     * @assumption If Community heating is present in an off-gasgrid Urban or
     * TownAndFringe dwelling, we assume that onGasGrid was answered incorrectly
     * in the survey and set it to true.
     *
     * @assumption If Community heating is present in an off-gasgrid rural
     * (Village or Hamlets and Isolated Dwellings) dwelling, it is assumed to
     * use wood biomass.
     *
     * @param model
     * @param heatSource
    ...
    /**
     * Get a central heating system from the technology model, or create one and
     * add it if there isn't one.
     *
     * @param tech
