 ...
 *
 * I have made many of the methods in this static, but that is only intended to
 * indicate that they are (a) pure functions, which don't involve keeping state
 * in the builder and (b) available to test without instantiating this class.
 *
...
public class HeatSourceBuilder implements IHeatSourceBuilder {
    ...
    /**
     * This is where the actual logic lives. I have made it static because it's
     * stateless, aside from responsiveness lookups.
     *
     * @assumption As per CHM, CPSUs are treated as standard boilers.
     * @assumption As per CHM, no keep hot facility is present.
     * @assumption If a boiler is present, but flue type was not specified,
     * assume balanced flue.
     * @assumption As per the CHM, the circulating pump of boilers with a pump
     * is outside.
     * @assumption Boilers are not weather compensated.
     * @assumption Boilers are not condensing unless we found a Sedbuk row
     * specifying them as such.
     * @assumption Boiler responsiveness set based on CHM table rather than SAP
     * table.
     *
     * @param model
     * @param dtoProvider
    ...
    private static IBoiler createBoiler(final FuelType fuelType, final IHeatingDTO dto, final BoilerType boilerType,
        ...
        switch (boilerType) {
            ...
            case STORAGE_COMBI:
                ...
                if (storeInsulationThickness == 0) {
                    storeInsulationThickness = 50d;
                }
     ...
     * @assumption Non-electric heat pump with no flue specified has a balanced
     * flue.
