 ...
 * </p>
 * <ul>
 * <li>{@link uk.org.cse.nhm.hom.emf.technologies.impl.CommunityHeatSourceImpl#isChargingUsageBased
 * <em>Charging Usage Based</em>}</li>
 * <li>{@link uk.org.cse.nhm.hom.emf.technologies.impl.CommunityHeatSourceImpl#getHeatEfficiency
 * <em>Heat Efficiency</em>}</li>
 * </ul>
 *
 * @generated
 */
public class CommunityHeatSourceImpl extends HeatSourceImpl implements ICommunityHeatSource {
    ...
    /**
     * The default value of the '{@link #isChargingUsageBased() <em>Charging
     * Usage Based</em>}' attribute.
     * <!-- begin-user-doc --> <!--
     * end-user-doc -->
     *
    ...
    /**
     * The flag representing the value of the '{@link #isChargingUsageBased()
     * <em>Charging Usage Based</em>}' attribute.
     * <!-- begin-user-doc -->
     * <!-- end-user-doc -->
     *
     * @see #isChargingUsageBased()
    ...
    /**
     * The default value of the '{@link #getHeatEfficiency() <em>Heat
    ...
    /**
     * The cached value of the '{@link #getHeatEfficiency() <em>Heat
     * Efficiency</em>}' attribute.
     * <!-- begin-user-doc --> <!--
     * end-user-doc -->
     *
    ...
    public void setChargingUsageBased(boolean newChargingUsageBased) {
        ...
        if (newChargingUsageBased) {
            flags |= CHARGING_USAGE_BASED_EFLAG;
        } else {
            flags &= ~CHARGING_USAGE_BASED_EFLAG;
        }
    ...
    protected void satisfySpaceDemand(final IConstants constants, final IEnergyState state, final double amount, final double controlFactor) {
        ...
        /**
         * This is the factor taken from SAP 2012 Table 4c (3) Glenn commented
         * this out TODO check why
         *
         *
         * final double controlFactor;
         *
         * if (calculatorType.heating == ECHeating.BREDEM2012) { controlFactor =
         * 1; } else if (isChargingUsageBased()) { // it seems like the
         * discriminating factor here is whether there are // TRVs in the space
         * heater. if
         * (controls.contains(HeatingSystemControlType.THERMOSTATIC_RADIATOR_VALVE))
         * { // 2310, 2306 controlFactor =
         * constants.get(CommunityHeatingConstants.LOW_SPACE_USAGE_MULTIPLIER);
         * log.debug("Control factor for usage-based thermostatic systems : {}",
         * controlFactor); } else { // 2308, 2309 controlFactor =
         * constants.get(CommunityHeatingConstants.MEDIUM_SPACE_USAGE_MULTIPLER);
         * log.debug("Control factor usage based non-thermostatic systems : {}",
         * controlFactor); } } else { if (systemIsThermostaticallyControlled) {
         * // Codes from table in this branch: // 2303, 2304, 2307, 2305
         * controlFactor =
         * constants.get(CommunityHeatingConstants.MEDIUM_SPACE_USAGE_MULTIPLER);
         * log.debug("Control factor for thermostatically controlled flate rate
         * : {}", controlFactor); } else { // codes in this branch // 2301, 2302
         * controlFactor =
         * constants.get(CommunityHeatingConstants.HIGH_SPACE_USAGE_MULTIPLER);
         * log.debug("Control factor for non-thermostatically controlled flat
         * rate : {}", controlFactor); } }
         *
         */
        /**
    ...
    protected void satisfyHotWaterDemand(final IConstants constants, final IEnergyState state, final double hw, final double gains, final double controlFactor) {
         ...
         * final double controlFactor;
         *
         * if (calculatorType.heating != ECHeating.BREDEM2012) { if
         * (isChargingUsageBased()) { controlFactor =
         * constants.get(CommunityHeatingConstants.LOW_WATER_USAGE_MULTIPLIER);
         * } else { controlFactor =
         * constants.get(CommunityHeatingConstants.HIGH_WATER_USAGE_MULTIPLIER);
         * } } else { // Hot water usage adjustments only apply to the SAP
         * calculator. controlFactor = 1; }
         */
        final double distributionLossFactor = constants.get(CommunityHeatingConstants.DEFAULT_DISTRIBUTION_LOSS_FACTOR);
     ...
     * will pay for.
     *
     * @param state the state to add some community fuel usage to (and/or
     * electricity generated, in CHP)
     * @param communityHeatRequired the amount of heat being consumed by the
     * household
     */
    ...
    public double getDemandTemperatureAdjustment(final IInternalParameters parameters, final EList<HeatingSystemControlType> controlTypes) {
        ...
        if (!isChargingUsageBased()) {
            if (!( // flat rate charging and no thermostatic control
     ...
     * exhibit additional tank-like losses for a 110l, 50mm factory insulated
     * tank
     */
