...
public class EObjectSerializer {
    ...
    /**
     * Every {@link EObject} is turned into a bit of BSON which contains the
     * attribute ___eclass___; this tells the {@link Reader} what class of
     * EObject to turn a thing into when deserializing
     */
    private static final String ECLASS_KEY = "___eclass___";
    /**
     * Since {@link EClass} instances are defined within an {@link EPackage}, we
     * need also to know which {@link EPackage} we're looking in. Since
     * {@link EPackage}s have an XMI schema with a globally unique namespace
     * URI, that is the best way to name a package.
     *
     * If an EObject is mapped which contains another EObject from the same
     * package, the namespace key will only be written for the container and not
     * the containee, to avoid wasting space.
     *
     * This string is used to denote the namespace int the bson.
     */
    private static final String NAMESPACE_KEY = "___ns___";
    /**
     * For the purposes of handling cross-references, every converted EObject is
     * given a unique ID within the serialization that is taking place. This is
     * then used to restore the cross-references when reading.
     *
    ...
    public static class Serializer<T extends EObject> extends StdSerializer<T> {
        ...
        /**
         * A helper class used to assign unique IDs to EObjects during the write
         * process, so as to allow cross-references to work
         *
         * @author hinton
         *
         */
        private static class Counter {
            ...
            /**
             * @param object
             * @return a unique int for object, which will always be the same
        ...
        private void addCrossReferences(final EObject in, final JsonGenerator out, final SerializerProvider sp, final Counter counter) throws JsonGenerationException, IOException {
            for (final EReference reference : in.eClass().getEAllReferences()) {
                ...
                if (reference.isUnsettable() && !in.eIsSet(reference)) {
                    continue;
                }
                ...
                if (reference.isMany()) {
                    ...
                    if (o != null) {
                        out.writeNumberField(reference.getName(), counter.get(o));
                    }
                }
            }
        }
        ...
        private void addAttributes(final EObject in, final JsonGenerator out, final SerializerProvider sp) throws JsonProcessingException, IOException {
            for (final EAttribute attribute : in.eClass().getEAllAttributes()) {
                ...
                if (attribute.isUnsettable() && !in.eIsSet(attribute)) {
                    continue;
                }
    ...
    public static class Deserializer<T extends EObject> extends StdDeserializer<T> {
        ...
        private static class XRef {
            ...
            /**
             * The index of the referent; that is the value to which its
             * {@link EObjectConverter#OBJECT_ID_KEY} property is mapped in the
             * BSON
             */
            ...
            /**
             * Using the index, restore {@link #reference} on {@link #target} to
             * point at theindex.get({@link #index})
             *
             * @param theindex the mapping from int to referent, built during
        ...
        private EObject deserialize(final JsonNode node, String namespace,
            ...
            if (p == null) {
                throw new RuntimeException("Cannot deserialize EObject from package " + namespace + " - namespace not registered");
            }

            /**
             * This is the classifier with name eClassName, if there is one in
             * the package
             */
            final EClassifier classifier = p.getEClassifier(eClassName);

            if (!(classifier instanceof EClass)) {
                throw new RuntimeException("Cannot find EClass with name " + eClassName + " in " + p);
            }
        ...
        private void prepareXRefs(final JsonNode node, final EObject obj, final ArrayList<XRef> xrefs) {
            for (final EReference reference : obj.eClass().getEAllReferences()) {
                ...
                if (node.has(reference.getName()) == false) {
                    continue;
                }
