...
/**
 * Describes the structural form of a house case. It is composed of two main
 * parts
 *
 * <ol>
 * <li> {@link Storey} instances - each of these describes one of the storeys of
 * the house, in terms of its perimeter, height, u-values etc.</li>
 * <li> {@link Elevation} instances - each of these describes the apertures in
 * one of the four elevations of the house, in terms of the percentage of
 * exterior walls that is glazed, or contains doors</li>
 * </ol>
 *
 * The {@link #addStorey(Storey)} and
 * {@link #setElevation(ElevationType, Elevation)} methods should be used to set
 * up the storeys and elevations in this structure.
 * <br />
 *
 * This class is intended to be easily serializable to MongoDB, so it oughtn't
 * have any cross-references between fields.
 * <br />
 *
 * At the moment it has no other information in it, but at some point it should
 * probably hold some details like draught lobby presence & other broad
...
public class StructureModel implements ICopyable<StructureModel> {
    ...
    /**
     * This is like the clone method, but I dislike the cloning interfaces for
     * the usual reasons
     *
    ...
    /**
     * This does\ a composition, going through each storey and asking it to
     * accept the visitor and present its heat loss surfaces and windows and so
     * on.
     * <br />
     *
     *
     * @assumption This method presumes that the maximum contact area is shared
     * between each storey, so heat loss area above and below is minimal.
     * <br />
     * This assumption makes it much easier to work out heat loss through the
     * roof and floor of a storey, as the alternative is to compute the
     * non-intersecting area between each polygon. Also, houses typically do
     * have this property (they usually don't look like this in plan:
     *
     * <pre>
     *[-------]
     *    [-------]
     ...
     * </pre>
     *
     * If you did have a house which looked like this, the heat loss from the
     * roof of the ground storey, the roof and floor of the first storey, the
     * roof and floor of the second storey and the floor of the top storey would
     * all be underestimated by this approach (they would be zero, because the
     * assumption makes a house which looks like this:
     *
     * <pre>
     *[-------]
     *[-------]
