...
/**
 * Represents a storey of a building in the house model. To set up a storey, you
 ...
 * </pre>
 * <br />
 * Calls to {@link #setPerimeter(Polygon)} will clear all existing state for the
 * storey's walls, and replace them with new empty walls according to the given
 * polygon. This is the only way to move the corners of the storey around.
 * <br />
 * Once the perimeter polygon has been set, you can iterate over the walls in it
 * with {@link #getWalls()}, and then use the {@link IMutableWall} interface to
 * alter the walls' attributes
 * <br />
 ...
 * </ul>
 *
 * TODO internal dimensions vs. external dimensions. TODO room in roof behaviour
...
public class Storey implements IStorey {
    ...
    /**
     * The segment data for each segment of this storey's perimeter. Each
     * segment data describes the end of a segment (so the segment's line goes
     * from the previous segment data in the list to the given one)
     */
    ...
    /**
     * What kind of storey this is - this affects how we present area to the
     * calculator.
     */
    private FloorLocationType floorLocationType;

    /**
     * The u-value for heat loss from this floor to void space or ground below
     * it
     */
    ...
    /**
     * This will destroy all the existing information about the storey, and
     * replace it with a new polygon with default values for all the segments.
     * Use {@link #getWalls()} to iterate through the walls and set their
     * u-values and construction types. Actually, that stuff could live in the
     * elevation as well. hmm.
     *
     * @param polygon
     */
    ...
    /**
     * Present the heat loss surfaces from this storey to the given
     * {@link IEnergyCalculatorVisitor}.
     *
     * @param visitor the visitor
     * @param elevations a map from elevation type to elevation, which should be
     * populated for all four elevations
     * @param areaBelow the heated area underneath this storey (i.e. the area of
     * the storey below)
     * @param areaAbove the heated area above this storey (i.e. the area of the
     * storey above)
     */
    public void accept(
    ...
    /**
     * Modify {@link #getCeilingUValue()} by adding a layer of insulation which
     * has the stated r-value
     *
     * @param rValue
     * @since 1.3.4
     */
    ...
    public double getLength(final ElevationType elevationType) {
        double accumulator = 0;
        for (final IWall wall : getImmutableWalls()) {
            if (wall.getElevationType() == elevationType) {
                accumulator += wall.getLength();
            }
        }
        return accumulator;
    ...
    /**
     * Iterates through all the walls in this storey, showing them to the
     * visitor as heat loss areas.
     *
     * @param visitor
     * @param elevations
     */
    ...
    /**
     * This is used by the {@link SegmentWrapper} class to implment
     * {@link IMutableWall#split(double)}; it inserts a new point into the
     * polygon after the given existing point.
     *
     * @param after
     * @param inserted
     */
        ...
        if (exposedPerimeterCache < 0) {
            ...
            for (final IWall wall : getImmutableWalls()) {
                if (wall.getWallConstructionType().getWallType() == WallType.External) {
                    exposedPerimeter += wall.getLength();
                }
            }
    ...
    /**
     * @return a weighted average of wall thickness, as specified in the yellow
     * box underneath SAP 2012 S3.
     */
