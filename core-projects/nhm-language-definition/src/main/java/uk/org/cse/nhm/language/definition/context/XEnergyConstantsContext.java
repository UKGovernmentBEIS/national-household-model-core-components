...
public class XEnergyConstantsContext extends XContextParameter {
	...
	public enum XEnergyConstantType {
		...
		@Doc("This is the mean loss factor for overshading (an array) - default is 1.000, 1.000, 0.830, 0.670, 0.500 ")
		 LightingConstants09_OVERSHADING_ACCESS_FACTORS(5),
		...
		@Doc("This converts from M3/hr of air changes to W/degree temperature difference. - default is 0.330 ")
		 EnergyCalculatorContstants_VENTILATION_HEAT_LOSS_COEFFICIENT(1),
		@Doc("This is the number of extra air changes per hour caused by storeys of the house above the ground floor - default is 0.100 ")
		 EnergyCalculatorContstants_STACK_EFFECT_AIR_CHANGE_PARAMETER(1),
		@Doc("This is the number of air changes per hour caused by not having a draught lobby (double front doors) - default is 0.050 ")
		 EnergyCalculatorContstants_DRAUGHT_LOBBY_AIR_CHANGE_PARAMETER(1),
		@Doc("The wind exposure factor is divided by this number - default is 4.000 ")
		 EnergyCalculatorContstants_WIND_FACTOR_DIVISOR(1),
		@Doc("This is the amount below demand temperature a fully unresponsive heating system is assumed to provide (defined in BREDEM 9.2.2) - default is 2.000 ")
		 EnergyCalculatorContstants_UNRESPONSIVE_HEATING_SYSTEM_DELTA_T(1),
		@Doc("The gains per occupant, in watts due to metabolism (BREDEM 8.2 defines this) - default is 60.000 ")
		 EnergyCalculatorContstants_METABOLIC_GAINS_PER_PERSON(1),
		@Doc("The gains per occupant due to evaporation, in watts - default is 40.000 ")
		 EnergyCalculatorContstants_EVAPORATION_GAINS_PER_PERSON(1),
		@Doc("The thermal bridging coefficient used to estimate the thermal bridge contribution to heat loss parameter, for buildings which are older than the improvement year - default is 0.150 ")
		 EnergyCalculatorContstants_OLD_THERMAL_BRIDGING_COEFFICIENT(1),
		@Doc("The thermal bridging coefficient used to estimate the thermal bridge contribution for buildings which are built in or after the improvement year - default is 0.038 ")
		 EnergyCalculatorContstants_NEW_THERMAL_BRIDGING_COEFFICIENT(1),
		@Doc("The year in which the thermal bridging coefficient switches from {@link #OLD_THERMAL_BRIDGING_COEFFICIENT} to {@link #NEW_THERMAL_BRIDGING_COEFFICIENT} - default is 2003.000 ")
		 EnergyCalculatorContstants_THERMAL_BRIDING_COEFFICIENT_IMPROVEMENT_YEAR(1),
		@Doc("The multipliers for OvershadingType which affect the degree of solar gains. - default is 1.000, 1.000, 0.770, 0.540, 0.300 ")
		 EnergyCalculatorContstants_SOLAR_GAINS_OVERSHADING(5),
		@Doc("The reflection factor in solar gains (how much solar gain is lost because of reflectivity) - default is 0.900 ")
		 EnergyCalculatorContstants_SOLAR_GAINS_REFLECTION_FACTOR(1),
		...
		@Doc("The amount below the demand temperature the threshold temperature is for gains utilisation factor calculation - default is 4.500 ")
		 EnergyCalculatorContstants_GAINS_UTILISATION_FACTOR_THRESHOLD_DIFFERENCE(1),
		@Doc("The air change rate due to doors and windows for a house where none of the doors and windows are draught stripped - default is 0.250 ")
		 EnergyCalculatorContstants_WINDOW_INFILTRATION(1),
		@Doc("The reduction in air change rate due to doors and windows for a house where all of the doors and windows are draught stripped - default is 0.200 ")
		 EnergyCalculatorContstants_DRAUGHT_STRIPPED_FACTOR(1),
		@Doc("The reduction in effective local wind speed due to having 1 sheltered side - default is 0.075 ")
		 EnergyCalculatorContstants_SHELTERED_SIDES_EXPOSURE_FACTOR(1),
		...
		@Doc("Demand temperature adjustment by storage heater control type (manual, automatic, celect) - default is 0.300, 0.000, 0.000 ")
		...
		@Doc("The high rate fraction for a space-heating ground source heat pump with on-peak auxiliary direct electric - default is 1.000, 0.800, 0.600 ")
		 SplitRateConstants_GROUND_SOURCE_SPACE_HEAT_WITH_ON_PEAK_AUXILIARY(3),
		@Doc("The high rate fraction for a space-heating ground source heat pump without on peak auxiliary - default is 1.000, 0.700, 0.600 ")
		 SplitRateConstants_GROUND_SOURCE_SPACE_HEAT_NO_AUXILIARY(3),
		...
		@Doc("Solar overshading factor by overshading type - default is 1.000, 1.000, 0.900, 0.800, 0.500 ")
		 SolarConstants_OVERSHADING_FACTOR(5),
		...
		@Doc("The average wattage of the circulation pump - default is 8.556 ")
		 SolarConstants_CIRCULATION_PUMP_WATTAGE(1),
		@Doc("The temperature adjustment for a direct electric non-thermostatic heater - default is 0.500 ")
		 HeatingSystemConstants_DIRECT_ELECTRIC_NONTHERMOSTATIC_T_ADJ(1),
		@Doc("The worst-case primary pipework losses for a heating system - default is 139.100 ")
		 HeatingSystemConstants_BAD_PRIMARY_PIPEWORK_LOSSES(1),
		@Doc("The medium-case primary pipework losses for a heating system - default is 69.580 ")
		 HeatingSystemConstants_MED_PRIMARY_PIPEWORK_LOSSES(1),
		@Doc("The best-case primary pipework losses for a heating system - default is 41.000 ")
		 HeatingSystemConstants_GOOD_PRIMARY_PIPEWORK_LOSSES(1),
		@Doc("Additional losses caused by the presence of a keep hot facility on a combi boiler - default is 25.000, 95.000, 25.000 ")
		 HeatingSystemConstants_KEEP_HOT_FACILITY_PIPEWORK_LOSSES(3),
		...
		@Doc("The wattage of the central heating circulation pump - this is from SAP table 4f, converted into watts and assigned over the 9 heating months equally - default is 11.123 ")
		...
		@Doc("The wattage of oil boiler fuel pump - from SAP table 4f, converted into watts and distributed over the 9 heating months - default is 8.556 ")
		...
		@Doc("The wattage of a solar heating pump - from SAP table 4f, assuming solar pump runs equally all year - default is 8.556 ")
		 PumpAndFanConstants_SOLAR_WATER_HEATING_PUMP_WATTAGE(1),
		...
		@Doc("The volume multiplier for a warm air system's circulating fan - default is 0.091 ")
