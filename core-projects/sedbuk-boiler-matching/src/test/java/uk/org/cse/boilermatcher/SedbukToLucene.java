...
public class SedbukToLucene {
	...
	public void testLuceneIndex() throws IOException {
		try(BufferedReader questions = 
			...
			while ((row = questions.readLine()) != null) {
				...
				if (parts.length > 1) index.find(parts[0], parts[1], null, null, null);
			}
		}
	}
	...
	@Test
	/**
	 * We allow some leeway with this metric since the test itself is slightly wrong: it doesn't account for matches which are essentially the same, but with the words in a different order.
	 * 
	 * @throws IOException
	 */
