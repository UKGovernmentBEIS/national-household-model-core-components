...
/**
 * This is a class which generates subclasses of {@link InvocationReader} which
 * handle individual mapped types. What each generated class does is implement
 * the abstract read method and (a) new up the mapped type and then (b) generate
 * callbacks for all its fields so that they get set in the end.
 *
 * The consequences of doing this with codegeneration rather than reflection are
 * good and bad;
 *
 * good: it's much faster, and stack traces are a lot easier to follow because
 * you can see what kind of thing is being constructed rather than getting a lot
 * of recursive calls to apparently identical methods which have different
 * runtime behaviour bad: it's a bit harder to maintain, and it makes various
 * methods not refactor-safe in some other classes, specifically
 * {@link Invocation}, {@link IReadContext} and {@link InvocationReader}
 *
 * @author hinton
 *
 * @param <T>
...
class InvocationReaderLoader<T> extends ClassLoader implements Opcodes {
    ...
    /**
     * Defines the class which will actually construct a thing of the desired
     * type
     *
     * @param properties
    ...
    /**
     * If the type we are reading is an inner class, we need to register it as
     * an inner class.
     *
     * @param cw
     */
