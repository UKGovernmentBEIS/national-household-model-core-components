...
public interface INationalHouseholdModel {
    ...
    /**
     * Convert the given scenario into an include graph. This is also included
     * in a validation result for free.
     */
    public <P> IIncludeGraph<P> getIncludeGraph(final IFS<P> fs, final P top);

    /**
     * Extract all the definitions from the given scenario. This is also
     * included in a validation result for free.
     */
    public <P> Set<IDefinition<P>> getDefinitions(final IFS<P> fs, final P top);

    /**
     * Run a simulation talking to the given callback. This method is
     * synchronous, so you probably want to spawn a thread to do it in.
     */
    ...
    /**
     * Take all of a scenario at the given path and crunch it down into the bare
     * essentials needed to run the model.
     */
