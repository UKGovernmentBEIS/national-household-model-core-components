 ...
 *
 * If this test fails, it's because you've changed an internal enum but not
 * amended the language enum to correspond to it.
 *
 * At some point it might be reasonable to relax the 1-1 correspondence in the
 * test
 *
 * @author hinton
 *
...
public class MapEnumTest {
    ...
    /**
     * For all the static methods in {@link MapEnum}, this checks that (a) they
     * map from an enum to an enum and (b) they are a 1-1 exhaustive mapping.
     *
    ...
    public void testMapEnumAllSides() throws IllegalAccessException, IllegalArgumentException, InvocationTargetException {
        for (final Method m : MapEnum.class.getMethods()) {
            if (m.getName().equals("energyConstant")) {
                continue;
            }
