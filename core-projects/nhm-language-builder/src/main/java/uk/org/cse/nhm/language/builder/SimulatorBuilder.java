...
public class SimulatorBuilder implements ISimulatorBuilder {
    ...
    /**
     * This is the guice injection scope for things annotated with
     * {@link SimulationScoped}. It ensures that within this thread, during a
     * call to build, we only make one of each {@link SimulationScoped} thing.
     *
     * It is unrelated to the {@link IAdaptingScope}, which is not to do with
     * dependency injection
     */
    private final SimpleScope simulationScope;

    /**
     * Visitor which adds all the adapters in {@link #adapters} into all the
     * things it visits
     *
    ...
    public ISimulator build(final XScenario scenario,
        ...
        try {
            ...
            final Set<Object> eager = injector.getInstance(Key.get(new TypeLiteral<Set<Object>>() {
            }, Eager.class));
            ...
            if (th instanceof RuntimeException) {
                ...
                throw new RuntimeException("Exception caught building simulator", th);
            }
        }
    }
    ...
    private void configureGlobalVariables(final XScenario scenario, final String executionID) {
        ...
        simulationScope.seed(new Key<List<String>>(StockID) {
        }, scenario.getStockID());
        ...
        simulationScope.seed(Key.get(
                new TypeLiteral<Function<Double, List<Double>>>() {
    ...
    /**
     * Takes the energy calculator parameters out of the scenario's parameters
     * context, and creates an {@link OverrideConstants} with them, or just
     * returns {@link DefaultConstants#INSTANCE} if there are no constants
    ...
    /**
     * Turn the given string into a double, or a double array, or throw a number
     * format exception.
     *
     * @param value
     * @return
