...
public class StructuralInfiltrationAccumulatorTest {
	...
	public void testNothing() {
		...
		accumulator.calculateAirChangeRate(houseCase, parameters);
		Assert.assertEquals("Infiltration is window only if nothing is presented",
                            0.25, accumulator.getAirChangeRate(), 0.01);
	}
	...
	public void testWallAccumulation() {
		...
		Assert.assertEquals("Biggest wall infiltration wins",
                            1.25, accumulator.getAirChangeRate(),
                            0.01);
	}
	...
	public void testWallAccumulation2() {
		...
		Assert.assertEquals("Biggest wall wins",
                            2.25, accumulator.getAirChangeRate(),
                            0.01);
	}
	...
	public void testStackEffect() {
		...
		Assert.assertEquals(
				"Stack effect parameter applies",

				0.25 + 3 * DefaultConstants.INSTANCE.get(EnergyCalculatorConstants.STACK_EFFECT_AIR_CHANGE_PARAMETER),
				accumulator.getAirChangeRate(),
				0.01);
	}
