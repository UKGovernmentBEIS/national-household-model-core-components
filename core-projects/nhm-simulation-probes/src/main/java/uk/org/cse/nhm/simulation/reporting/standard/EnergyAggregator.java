...
/**
 * A report aggregator which sums up national energy usage for the purpose of making a graph at the end.
 * 
 * It is inefficient, but safe - rather than listening to the state for state changes, it just computes
 * the power usage at the end of each step and integrates over the step.
 * 
 * @author hinton
 *
 */
public class EnergyAggregator implements ISimulationStepListener {
	...
	public void simulationStepped(final DateTime dateOfStep, final DateTime nextDate, final boolean isFinalStep) throws NHMException {
		...
		if (dateOfStep.equals(nextDate)) return;
	...
	/**
	 * Takes the given array of power values, in energy/year, and converts to energy by multiplying out the number of years
	 * between the two datetimes given.
	 * 
	...
	/**
	 * Takes a collection of dwellings and accumulates their power by service type and fuel type into the two given arrays, in kWh/year
	 * @param dwellings
	 * @param byService
	 * @param byFuel
	 */
