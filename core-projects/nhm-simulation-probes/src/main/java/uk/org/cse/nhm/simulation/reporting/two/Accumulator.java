...
public abstract class Accumulator extends AbstractNamed {
    ...
    public static class Mean extends Accumulator {
        ...
        public IAccumulation doStart() {
            return new DoubleAccumulation() {
                ...
                @Override
                public void doPut(double w, double d) {
                    sx += (d * w);
                    sw += w;
                }
    ...
    /**
     * medians cannot be computed in a streaming manner, so we have to store
     * them all. a bit pants.
     */
    public static class Tile extends Accumulator {
        ...
        public IAccumulation doStart() {
            return new DoubleAccumulation() {
                ...
                @Override
                public void doPut(double w, double d) {
                    values.add(new double[]{d, w});
                }

                @Override
                public double get() {
                    return NTile.evaluate(p, values.toArray(new double[values.size()][]));
                }
            };
        }
    }
    ...
    /**
     * Algorithm after West (1979), D. H. D. West (1979). Communications of the
     * ACM, 22, 9, 532-535: Updating Mean and Variance Estimates: An Improved
     * Method
     */
