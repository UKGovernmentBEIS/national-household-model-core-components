...
/**
 * This group implements the three basic set operations, UNION, DIFFERENCE and
 * INTERSECTION.
 *
...
public class SetOperationDwellingGroups extends BaseDwellingGroup {
    ...
    public enum Operation {
        /**
         * If this operation is selected, the group will contain all elements in
         * any of its source groups
         */
        UNION,
        /**
         * If this operation is selected, the group will contain all the
         * elements common to all its source groups
         */
        INTERSECTION,
        /**
         * If this operation is selected, the group will contain all the
         * elements in its first source group which are in none of the
         * subsequent source groups.
         */
        DIFFERENCE
    }
            ...
            case UNION:
                final IDwellingGroupListener unionListener = new IDwellingGroupListener() {
                    ...
                    public void dwellingGroupChanged(IStateChangeNotification cause, IDwellingGroup source, Set<IDwelling> added, Set<IDwelling> removed) {
                        ...
                        if (checkAwaitingModification(modCounts, source, sources)) {
                            return;
                        }
    ...
    /**
     * Returns true if we are waiting for another modification to come through,
     * and so shouldn't send out notifications right now
     *
     * @param modCounts the modification counts which for the groups in sources;
     * this will be updated
     * @param thisGroup the group which has notified us that it's changed
     * @param sources all the groups we are interested in
     * @return true iff the only group whos modcount is out of date in modCounts
     * is thisGroup.
     */
