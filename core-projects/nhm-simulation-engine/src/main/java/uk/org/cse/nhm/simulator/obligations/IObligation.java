...
public interface IObligation extends IComponentsAction {
    ...
    /**
     * @param state
     * @return the payments associated with the given state of affairs for a
     * dwelling;
     * <p>
     * note that it is the duty of the obligation to detect the case in which
     * the given date does not match the state's date, and if necessary mock up
     * a new state for any delegates which use the time dimension. you have been
     * warned.
     */
    public Collection<IPayment> generatePayments(final DateTime onDate, final IComponentsScope state);

    /**
     * Invoked to tell the obligation to consider itself "real" and to schedule
     * itself to act on the given dwelling
     */
