...
public class OrderedChoiceAction extends AbstractNamed implements IStateAction {
	...
	/**
	 * This represents the options for a dwelling; it holds the values for each alternative,
	 * and keeps track of the best one.
	 */
	static final class DwellingOptions implements Comparable<DwellingOptions> {
		...
		int bestIndex;
		/**
		 * 1 if the objective is better when it is smaller; -1 if it is better when larger.
		 * Used to multiply objective values before storing them.
		 */
		final double mul;
		/**
		 * True if the values of best and bestIndex are up-to-date. This is cleared by
		 * {@link #set(int, double)}, and set by {@link #update()}.
		 * 
		 * {@link #update()} is only called by {@link #any()}, so you <emph>MUST</emph> call 
		 * {@link #any()} before using the other values, which would include touching the comparator.
		 */
		boolean clean = true;
		/**
		...
		/**
		 * A random int, which is not reused in any other {@link DwellingOptions} being considered,
		 * so we can use it for tie breaking in a reliable way.
		 */
		...
		/**
		 * This comparator compares the best objective value first; if two houses have the same
		 * best objective value, we use a random int to break the tie.
		 * @param o
		 * @return
		 */
		@Override
	...
	public Set<IDwelling> apply(
		...
		// generate the initial state for the PQ
		{
			...
			for (int i = 0; i<dwellings.size(); i++) randoms.add(i);
	...
	/**
	 * The suitable dwellings are those for which at least one action appears suitable.
	 * Some dwellings may ultimately be unsuitable, because of interaction between the actions.
	 */
	@Override
