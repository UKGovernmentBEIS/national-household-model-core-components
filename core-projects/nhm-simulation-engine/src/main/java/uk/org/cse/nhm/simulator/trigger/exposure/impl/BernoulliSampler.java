 ...
 *
 * NOTE: Similar to RandomHouseInstanceGroupSampler however sample size is
 * variable and exposure rate must be applied to each IDwelling individually.
 *
 * TODO:	To prevent a dwelling appearing in multiple samples, a list of
 * nonCandidates should be maintained as in RandomHouseInstanceGroupSampler.
 * This could be added as a class which extends this one
 * "DecisionMakingFrequencySamplerDistinctImpl".
 *
 * TODO:	Random member variable could be set to a class extending Random with a
 * particular distribution from which to sample
 *
 * e.g. class NormalRandom extends Random { public NormalRandom(Integer seed,
