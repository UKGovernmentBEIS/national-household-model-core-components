...
/**
 * This is a base class for {@link IDwellingGroup} implementors which provides boilerplate functionality, primarily
 * <ol>
 * <li>A set for tracking what is in the group</li>
 * <li>The {@link #update(IStateChangeNotification, LinkedHashSet, LinkedHashSet)} method, which will post notifications to listeners if necessary</li>
 * <li>A track of modification count, for efficient implementation of clients</li>
...
public abstract class BaseDwellingGroup extends AbstractNamed implements IDwellingGroup {
	/**
	 * This set contains all the house instances that are in this group. It's a {@link LinkedHashSet} to guarantee repeatable
    ...
    /**
     * Each time a subclass invokes {@link #update(IStateChangeNotification, LinkedHashSet, LinkedHashSet)} in a way that makes a change,
    ...
    /**
     * Subclasses can invoke this when they want to change the group contents and notify any listeners.
     * 
     * The method will intelligently preprocess the added and removed items so things which are already in the group are not
     * double-added and items which are not in the group are not double-removed.
     * 
     * Listeners are only notified if the contents set is actually changed.
     * 
     * Before listeners get notified, the contents will be updated to reflect the new state
     * @param cause TODO
