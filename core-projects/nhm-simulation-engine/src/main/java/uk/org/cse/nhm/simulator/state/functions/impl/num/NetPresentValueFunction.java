...
/**
 * A ranking function which computes the net present value of a house, given a
 * certain discount rate and time horizon.
 *
 * The NPV is calculated as
 *
 * <ol>
 * <li>Capital costs (installation costs etc)</li>
 * <li>Operational costs which are sum for year from 0 to
 * {@link #horizonInYears}-1
 * <ul>
 * <li>cost in year * discountRate ^ year</li>
 * <li>annual cost is fuel cost at current energy usage for 1 year + annual
 * maintenance costs of current state</li>
 * </ul>
 * </li>
 * </ol>
 *
...
public class NetPresentValueFunction extends ObligationForecastingFunction {
    ...
    /**
     * This is the discount rate per annum, so year n is priced at
     * (cost)*(discountRate)^(n-1) (if this year n = 1)
     */
    ...
    /**
     * TODO This is grim; since NPV depends on evaluating obligations, it could
     * depend on any of the dimensions on a house so for correctness we must
     * report that we depend on all dimensions. Hopefully NPV will not be used
     * in any performance critical change-driven setting (like a group defn.)
     */
    @Override
