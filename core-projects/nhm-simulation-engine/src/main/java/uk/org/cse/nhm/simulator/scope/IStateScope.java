...
public interface IStateScope extends IScope<IStateChangeSource> {
    ...
    /**
     * Modify the state, using the given action on each dwelling in the given
     * set of dwellings
     *
     * @return the affected dwellings
     */
    ...
    /**
     * Get a components scope for the given dwelling, if one has been created by
     * applying an action.
     */
    public Optional<IComponentsScope> getComponentsScope(final IDwelling dwelling);

    /**
     * Creates a free-floating hypothetical state about this house, which can
     * never become the truth but can be used to work stuff out
     */
    IHypotheticalComponentsScope createHypothesis(final IDwelling dwelling);

    /**
     * Find the 'prior scope' for this scope This is the scope for the parent
     * branch.
     */
