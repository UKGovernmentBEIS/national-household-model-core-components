...
/**
 * A default implementation of {@link IBranch}, which is created by {@link CanonicalState} or by another instanceof {@link BranchState}
 * when their {@link #branch(int)} method is invoked.
 * 
 * @author hinton
 *
 */
public class BranchState implements IBranch {
	...
	/**
	 * A map from the global, singleton dimension handle for each dimension in this simulation, to the branch-specific
	 * forked version of that dimension in this branch.
	 */
	@SuppressWarnings("rawtypes")
	final IInternalDimension[] branchedDimensions;
	
	/**
	 * A flag used to temporarily make {@link #getCreated()}, {@link #getModified()}, {@link #getDestroyed()} and {@link #getAnnotations(IDwelling)}
	 * return nothing, so that when a child branch is being applied back to this branch we can detect just those changes for which
	 * the child branch is responsible.
	 */
	...
	/**
	 * Given a dimension from the {@link IState#getDimensions()} on the {@link ICanonicalState} for this simulation,
	 * returns the branched dimension within this branch
	 * 
	...
	public <T> void set(final IDimension<T> dimension, final IDwelling instance, final T value) {
		if (value == null) log.warn("Setting {} to null for {}", dimension, instance);
	...
	public Set<IDwelling> getModified() {
		if (isolateChildren) return Collections.emptySet();
	...
	public void destroyDwelling(final IDwelling lenny) {
		if (created.contains(lenny)) created.remove(lenny);
		else destroyed.add(lenny);
	}

	protected void dieIfCannotMergeWith(final IBranch child) {
		if (child.isHypothetical()) throw new IllegalArgumentException("A hypothetical branch should never be merged");
	}
	...
	/**
	 * Copy the value for the given dimension of the affected dwellings in the given state into this state.
	 * @param dimension
	 * @param state
	 * @param affected
	 */
