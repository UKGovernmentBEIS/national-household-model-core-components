...
public class ComponentsScope extends Scope<IStateChangeSource> implements ISettableComponentsScope {
    ...
    /**
     * This is a shared, mutable datastructure held in common between all the
     * scopes in a branch; it is probably the worst bit of this.
     *
     * It is managed by callers to
     * {@link #createChild(IStateChangeSource, IBranch, Map)} below, who are
     * expected to pass in a map which should be worked on within the resulting
     * scope, and are also expected to handle merging the changes back into any
     * other maps should the child scope be added to this scope in the end.
     *
     * At the moment, said methods are
     * {@link #applyInSequence(List, ILets, boolean)}, {@link #apply(Set, ILets, uk.org.cse.nhm.simulator.scope.ISettableComponentsScope.IPicker)},
    ...
    /**
     * This is called in various places to create a sub-scope. Different
     * subclasses of this want to return class specific children. However, they
     * must all be forced to pass down "this" as the parent scope, if the branch
     * for the child scope is not the branch for this scope. So, the actual
     * subclass behaviour is in doCreateChild, and this is final and offers the
     * correct parent scope for the child.
     */
