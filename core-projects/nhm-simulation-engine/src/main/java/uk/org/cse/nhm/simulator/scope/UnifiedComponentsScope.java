...
class UnifiedComponentsScope implements IComponentsScope {
    ...
    public int getDwellingID() {
        return components.getDwellingID();
    }

    @Override
    public IDwelling getDwelling() {
        return components.getDwelling();
    }
    ...
    public IHypotheticalComponentsScope createHypothesis() {
        return components.createHypothesis();
    }

    @Override
    public <T> Optional<T> getGlobalVariable(String string, Class<T> clazz) {
        return components.getGlobalVariable(string, clazz);
    }

    @Override
    public GlobalTransactionHistory getGlobalAccount(final String accountName) {
        return components.getGlobalAccount(accountName);
    }

    @Override
    public void yield(final String key, final double value) {
        components.yield(key, value);
    }

    @Override
    public void unYield(String key) {
        components.unYield(key);
    }

    @Override
    public Optional<Double> getYielded(final String key) {
        return components.getYielded(key);
    }

    @Override
    public Map<String, Double> getYieldedValues() {
        return components.getYieldedValues();
    }

    @Override
    public void yieldAll(final Map<String, Double> values) {
        components.yieldAll(values);
    }
