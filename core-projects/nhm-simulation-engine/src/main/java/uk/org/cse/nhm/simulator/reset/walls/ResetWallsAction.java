...
public class ResetWallsAction extends AbstractNamed implements IComponentsAction {
    ...
    static class WallPropertyFunction extends AbstractNamed implements IComponentsFunction<double[]> {
        ...
        public double[] compute(final IComponentsScope scope, final ILets lets) {
            ...
            if (wall.getWallConstructionType().getWallType() == WallType.External) {
                if (uValue != null) {
                    result[0] = uValue.compute(scope, lets).doubleValue();
                } else {
                    result[0] = wall.getUValue();
                }
                if (infiltration != null) {
                    result[1] = infiltration.compute(scope, lets).doubleValue();
                } else {
                    result[1] = wall.getAirChangeRate();
                }
                if (thickness != null) {
                    result[2] = thickness.compute(scope, lets).doubleValue();
                } else {
    ...
    /**
     * I apologise for the grimness of this hack; this magic string is used to
     * bind the wall into the let scope to allow special wall functions to read
     * values out of the wall elsewhere in the system.
     */
