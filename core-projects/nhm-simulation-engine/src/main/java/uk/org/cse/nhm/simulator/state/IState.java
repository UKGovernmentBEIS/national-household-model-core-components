...
/**
 * An {@link IState} describes a possible configuration of the {@link IDwelling}s in the simulator.
 * At any given time there will be a single <em>canonical</em> {@link IState} describing the current
 * true configuration of all things. Along with this there may be several hypothetical states existing
 * containing possible futures which may be made canonical.
 * 
 * @author hinton
 *
 */
public interface IState {
	/**
	 * @return all of the dimensions that are available for {@link IDwelling}s in this state
	 */
	 ...
	 * @param dimension
	 * @param instance
	 * @return the generation counter for this dimension on this instance. You can use this to find out
	 * whether a particular dimension has changed since you last looked.
	 */
    ...
    /**
     * @return The random number utility for this branch; random number streams branch and
     * merge along with the state of the world, so a branch which is discarded can use all
     * the random numbers it likes without affecting random events in its parent branch.
     */
