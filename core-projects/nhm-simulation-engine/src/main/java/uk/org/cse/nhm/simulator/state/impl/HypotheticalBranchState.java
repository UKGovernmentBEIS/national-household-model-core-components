...
public class HypotheticalBranchState extends BranchState implements IHypotheticalBranch {
     ...
     * or delegates to another dimension. Any dimension you pass in here must be
     * able to be emplaced back into another branch by use of its branch()
     * method; that is to say, no replacement dimension should keep any ancestry
     * when it is branched.
     */
    @Override
    ...
    protected void dieIfCannotMergeWith(final IBranch child) {
        if (!child.isHypothetical()) {
            throw new IllegalArgumentException("Only hypothetical branches should ever be merged here");
        }
    }
