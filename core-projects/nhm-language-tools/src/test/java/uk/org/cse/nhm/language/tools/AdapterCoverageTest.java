...
/**
 * This test ensures that there is no situation where a scenario simply cannot
 * be built, because there is a potential requirement that A is adapted to B
 * somewhere, but no possible adapter that can make an A into a B anywhere.
 *
 * @author hinton
 *
 */
...
//			}
        ...
        private boolean isAssignableOrConvertable(final Class<?> from, final Class<?> to) {
            ...
            return (IComponentsAction.class.isAssignableFrom(from) && to.isAssignableFrom(IStateAction.class))
        ...
        private boolean verify(final Method m, final boolean outerHasNext) {
            ...
            for (int i = 0; i < m.getParameterTypes().length; i++) {
                ...
                if (TypeUtils.isAssignable(propertyType, parameterType)) {
                    continue;
                }
