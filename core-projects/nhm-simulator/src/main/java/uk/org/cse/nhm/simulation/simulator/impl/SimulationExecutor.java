...
public class SimulationExecutor implements ISimulationExecutor {
	 ...
	 * 
	 * 
	 * @see uk.org.cse.nhm.ipc.api.tasks.ISimulationExecutionService#run(java.util.UUID,
	 *      uk.org.cse.nhm.ipc.api.tasks.sim.IScenario,
	 *      ILets)
	 */
	@Override
			...
			try (final ISimulator simulator = run.build(xscenario, runID.toString(),
				...
				do {
					...
					if (completion == 0) {
						etc = new DateTime(
								(long)
								(now + 1000l * (45 + totalTime / (52 * Timer.ONE_WEEK)))); //45 seconds + 1 second per sim year 
					} else {
						etc = new DateTime(
								(long)
								(now + ((now - beginning) * ((1-completion) / completion))));
					}
