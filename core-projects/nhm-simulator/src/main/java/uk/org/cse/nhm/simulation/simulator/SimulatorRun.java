...
/**
 * This is a useful class for those who want to start a simulator - it
 * constructs a {@link SimulatorBuilder} with a set of default modules that
 * contain the normal scenario elements of interest, and provides the
 * {@link #createStandardParameters(LoggingService, HousePropertyStoreRepository, SimulationParameter...)}
 * method to illustrate the extra parameters to the
 * {@link #build(XScenario, String, ISurveyCaseDataService, SimulationParameter...)}
 * method that the standard modules depend upon.
 *
 * It also provides {@link #parseScenario(Reader)}
 *
 * It also provides convenient access to the dependency injector (which is a bit
 * hacky, but useful in test cases), and the list of all known XML elements, for
 * creating a JAX context, generating documentation and so on.
 *
 * @author hinton
 *
 */
public class SimulatorRun {
    ...
    /**
     * This is a useful static method which makes a DI context with all the
     * normal modules in, plus any extra modules (e.g. a debugging module) that
     * you pass.
     *
     * This is the single source of truth about the language for most parts of
     * the system, so if you make a new module, put it here.
     *
     * @param cacheSize
     *
     * @param extraModules
    ...
    /**
     * The standard modules have certain standard parameters which are expected
     * in
     * {@link #build(XScenario, String, ISurveyCaseDataService, SimulationParameter...)}
     * as the final argument. This method will create a parameter list with the
     * required parameters in.
     *
     * It is a convenience, to make it easy to detect where parameters will need
     * adding in the rest of the source - if you need to bind some other thing
     * as a simulation scoped entity, because you have added a new module above
     * which consumes something from outside, add it to here as well and the
