...
/**
 * A visitor interface for a house to accept; the energy calculator algorithm
 * can use this to collect all the inputs it needs from a house however that
 * house may be structured.
 *
 * Where a given part of the house implements more than one of these interfaces,
 * it should be passed to each applicable method, so a window will invoke
 * visitTransparentElement and visitFabricElement.
 *
 * @author hinton
 *
 */
public interface IEnergyCalculatorVisitor {
     ...
     * systems determine the background temperature in a house, typically using
     * a responsiveness parameter to interpolate between an ideal and worst-case
     * background temperature.
     *
     * The actual energy to heat conversion will be done by something presented
     * to {#visitEnergyTransducer(IEnergyTransducer)}.
     */
    void visitHeatingSystem(IHeatingSystem system, double proportion);

    /**
     * Return the proportion of heat provided by the heat system (put in when we
     * visit it).
     *
     ...
     * of the calculation is done by these - see the interface for more details.
     */
    public void visitEnergyTransducer(final IEnergyTransducer transducer);

    /**
     * Present a ventilation system to the algorithm; ventilation systems are
     * used to compute the air change rate through the house. At the moment,
     * most houses won't contain one of these, as full ventilation systems are
     * quite rare.
     */
     ...
     *
     * (Also see visitDoor and visitWindow, which add the u-values for those
     * elements.)
     *
     * @param glazingType
     ...
     *
     * @param horizontalOrientation the angle from the dot product of this
     * plane's normal and the north vector.
     * @param verticalOrientation the angle between the normal from this plane
     * and the vertical
     * @param overshading the degree of overshading for this transparent
     * element.
     */
    ...
    /**
     * Add the infiltration contribution from intermittent fans (something which
     * is forced)
     *
     ...
     *
     * (If the door is glazed, also see visitTransparentElement, which adds the
     * natural lighting and solar gains.)
     */
     ...
     *
     * (Also see visitTransparentElement, which adds the natural lighting and
     * solar gains.)
     */
