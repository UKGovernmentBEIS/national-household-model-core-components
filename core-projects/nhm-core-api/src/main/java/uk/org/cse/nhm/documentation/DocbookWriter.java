...
/**
 * Helper to write docbook. This could of course use SAX to talk to a
 * contenthandler, but writing that stuff is so annoying it is unmanageable, in
 * particular because we want to allow structured text to be written in the doc
 * strings themselves, so you'd need to hook up a delegating handler parsing
 * those and so on.
 *
 */
