...
/**
 * Provides a naive implementation of IScope; the operating methods are
 * {#root(Tag)} to create a new top-level scope, and {#child(Tag)} to create a
 * new child of a given scope.
 */
public class Scope<Tag> implements IScope<Tag> {
    ...
    /**
     * This list of lists contains all of the objects added into this scope.
     * It's a list of lists so that we can tell where objects come in relation
     * to child scopes; the 0th element contains elements added before any child
     * scope was created. The 1st list contains those elements added after the
     * first child scope was closed but before the second was opened, and so on.
     */
    ...
    private void removeEmptyChildren() {
        ...
        final Deque<ImmutableList.Builder<Object>> notesAccumulator = new LinkedList<>();

        final ImmutableList.Builder<Scope<? extends Tag>> childrenBuilder = ImmutableList.builder();
