...
/**
 * An integration test for the heat loss parameter. Test values are calculated in
 * heat_loss_testcase_1.rws
 *
 * @author hinton
 */
public class TestCase1 {
  ...
  public void testHouseWithoutHeatingOrHotWater() {
    ...
    final BasicCaseAttributes attrs =
        new BasicCaseAttributes(
            "aacode",
            1,
            1,
            RegionType.London,
            MorphologyType.HamletsAndIsolatedDwellings,
    ...
    final Polygon plan = new Polygon(new int[] {0, 7, 7, 0}, new int[] {0, 0, 10, 10}, 4);
    ...
    //		hc.setExposure(1.0);
    //		hc.setInterZoneSpecificHeatLoss(352.75); //TODO add this as inference within house case?
    // hmm.
    ...
    tech.getCookers().add(CookerImpl.createMixed());
    ...
    final BredemExternalParameters parameters =
        new BredemExternalParameters(
        		EnergyCalculatorType.BREDEM2012,
            ElectricityTariffType.FLAT_RATE, 21, Optional.<Double>absent(), Optional.of(3.0), 2);
    final IHeatingSchedule schedule = new DailyHeatingSchedule();

    final ISeasonalParameters climate =
        new BREDEMHeatingSeasonalParameters(
            MonthType.January,
            new IWeather() {
    ...
    final IEnergyCalculationResult state =
        calc.evaluate(sc, parameters, new ISeasonalParameters[] {climate})[0];
    ...
    Assert.assertEquals(
        expectedVolume,
        state.getEnergyState().getTotalDemand(EnergyType.DemandsHOT_WATER_VOLUME),
        0d);
    ...
    Assert.assertEquals(
        ...
        ,
        state.getEnergyState().getTotalDemand(EnergyType.DemandsHOT_WATER),
        0.1);

    // verify gains & usage for lights and cooking are correct
    final double electricityForLighting =
        state.getEnergyState().getTotalDemand(EnergyType.FuelPEAK_ELECTRICITY, ServiceType.LIGHTING)
            + state
                .getEnergyState()
                .getTotalDemand(EnergyType.FuelOFFPEAK_ELECTRICITY, ServiceType.LIGHTING);

    final double gasForCooking =
        state.getEnergyState().getTotalDemand(EnergyType.FuelGAS, ServiceType.COOKING);
    ...
    Assert.assertEquals(
        308.46, state.getEnergyState().getUnsatisfiedDemand(EnergyType.DemandsHEAT), 0.05);
