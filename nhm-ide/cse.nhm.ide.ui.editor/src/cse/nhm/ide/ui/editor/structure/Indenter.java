...
public class Indenter {
	/**
	 * This stores the current set of tab stops; each time we look at a line,
	 * we see what characters are on it and twiddle the tab stops
	 */
	static class Tabstops {
		...
		/**
		 * This is a helpful string containing as many spaces as the top of the tabstops stack
		 */
		String string = "";
		...
		/**
		 * Change the current topmost tabstop - this happens when
		 * we have pushed a tab on this line, and then there are some words after it
		 * so we get
		 * 
		 * (do something
		 * ^   ^ this is the new stop
		 * |
		 * this was the stop
		 */
		...
		public void pop() {
			...
			if (val == null) this.string = "";
			else this.string = rep(val);
		}
		...
		/**
		 * This is the main thing - we have seen a new line and we want to know what the indent should be on
		 * the next line when we come to it.
		 * 
		 * This works by:
		 * looping through each char in the line; if it changes indentation, we modify the indentation
		 * we also keep a within-line stack of whether we are after a start-paren and so whether we need to amend the last tabstop
		 * 
	...
	public static void indent(final IDocument document, final int fromLine, final int toLine) throws BadLocationException {
		final Tabstops stops = new Tabstops();
		
		for (int i = 0 ;i<=toLine; i++) {
			final IRegion region = document.getLineInformation(i);
			int k = region.getOffset();
			int lk = 0;
			while (k < region.getLength() + region.getOffset()) {
				final char c = document.getChar(k);
				if (!Character.isWhitespace(c)) break;
				k++;
				lk++;
			}
			final String restOfLine = document.get(k, region.getLength() - lk);
			
			if (i >= fromLine) {
				document.replace(region.getOffset(), lk, stops.string);
			}
			
			stops.eat(restOfLine);
		}
	}
