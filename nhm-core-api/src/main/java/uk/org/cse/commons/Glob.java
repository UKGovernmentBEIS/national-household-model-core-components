...
public class Glob implements Predicate<String> {
    ...
    public Optional<String> getLiteralValue() {
        if (isWild()) return Optional.absent();
    ...
    public static boolean requireAndForbid(final Collection<Glob> globs, final Collection<String> input) {
    	glob_loop: for (final Glob glob : globs) {
			if (glob.isNegated()) {
    ...
    /**
     * Given a collection of globs, produce a predicate which will only pass a set of strings if:
     * 
     *  - every positive glob has a matching string in the set
     *  - no negative glob has a matching string in the set
     * 
     * @param globs
     * @return
     */
    public static Predicate<Collection<String>> requireAndForbid(final Collection<Glob> globs) {
    	if (globs.isEmpty()) return Predicates.alwaysTrue();
