...
public class SurveyCaseBuilder implements ISurveyCaseBuilder {
	...
	public void addStep(final ISurveyCaseBuildStep step) {
		if (initialized) throw new UnsupportedOperationException("Steps should not be added after initialize() has been called");
		steps.add(step);
		if (logger.isDebugEnabled()) logger.debug("addStep(): added:{}", step.getIdentifier());
	}
	...
	public void initialize() {
		if (initialized) throw new UnsupportedOperationException("Should not be initialized twice");
		initialized = true;
