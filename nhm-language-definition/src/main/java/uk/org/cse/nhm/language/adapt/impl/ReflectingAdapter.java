 ...
 * {@link Adapt} will be used to determine a type that this can adapt.
 * <p>
 * The main work is done by {@link AdapterMethod}, which wraps one of the subclass' methods, picking
 * out the various annotations on them to work out what to do.
 * 
 * 
 * @author hinton
 *
...
public class ReflectingAdapter implements IAdapter {
	...
	/**
	 * This wraps a single {@link Method} on its containing class; in the constructor,
	 * it looks at the method's arguments and annotations to work out the following:
	 * <ol>
	 * 	<li> what kind of thing does the method adapt </li>
	 *  <li> is the adaptation inheritable, or just for the one type </li>
	 *  <li> what properties of the thing being adapted does the method consume </li>
	 *  <li> for those properties, do they themselves need any adapting (see {@link ParameterWrapper} and subtypes for this) </li>
	 * </ol>
	 * 
	 * The {@link #adapt(Object, IAdaptingScope)} is used to actually do the adapting.
	 * 
	 * This processes each {@link ParameterWrapper} in turn, and then invokes the method
	 * when it's finished.
	 * <p>
	...
	private class AdapterMethod {
		...
		private class AdaptParameterWrapper extends MethodParameterWrapper {
			...
			public Object get(final Object from, final IAdaptingScope scope) {
				...
				if (value == LEGAL_NULL) return value;
		...
		private class ListAdaptParameterWrapper extends MethodParameterWrapper {
			...
			public Object get(final Object beingAdapted, final IAdaptingScope scope) {
				...
				if (values == LEGAL_NULL) return LEGAL_NULL;
		...
		/**
		 * Handles parameters of adapt methods which need no conversion 
		 * (e.g. primitive type -> primitive type)
		 * 
		 * @author hinton
		 *
		 */
		private class MethodParameterWrapper extends ParameterWrapper {
			...
			public Object get(final Object beingAdapted, final IAdaptingScope scope) {
				try {
					...
					if (value == null) return LEGAL_NULL;
					else return value;
		...
		/**
		 * Handles parameters which expect just the object which is being adapted
		 * 
		 * @author hinton
		 *
		 */
		...
		/**
		 * Wraps another parameter wrapper so that if it gives a null or LEGAL_NULL,
		 * the value is Optional.absent(), and otherwise it is Optional.of(value);
		 * 
		 * @author hinton
		 *
		 */
		...
		/**
		 * Base class for the various parameter wrappers above; each parameter wrapper
		 * is used by the containing {@link AdapterMethod} to work out the value of an
		 * argument to its method.
		 * 
		 * @author hinton
		 *
		 */
		...
		/**
		 * The method annotated with {@link Adapt} that this instance is managing
		 */
		private final Method method;
		/**
		 * The type of thing that this method adapts (parameter to the {@link Adapt} annotation)
		 */
		...
		/**
		 * A list of keys, indexed by parameter index; if putScope[i] is not null, the
		 * value of the ith parameter is placed in the adapting scope under key putScope[i]
		 * <em>before</em> the method is invoked.
		 */
		private final String[] putScope;
		/**
		 * If the method itself is annotated with {@link PutScope}, this is not null, and
		 * the value being adapted is put into the scope.
		 * 
		 * This would be the same as having a parameter for the type being adapted, and marking
		 * it with {@link PutScope}
		 */
		...
		/**
		 * If true, store the result in the global cache against the adapted element.
		 * This is taken off the {@link Adapt} annotation.
		 */
			...
			for (final Method m : clazz.getMethods()) {
				if (m.getReturnType() != Void.TYPE && m.getParameterTypes().length == 0) {
					if (m.getName().startsWith("get")) {
						if (m.getName().substring(3).equalsIgnoreCase(value)) return m;
					} 
					if (m.getName().startsWith("is")) {
						if (m.getName().substring(2).equalsIgnoreCase(value)) return m;
					} 
					if (m.isAnnotationPresent(Prop.class)) {
						if (m.getAnnotation(Prop.class).value().equalsIgnoreCase(value)) return m;
					}
				}
			}
		...
		public Object adapt(final Object from, final IAdaptingScope scope) {
			...
			for (final ParameterWrapper pw : parameters) {
				...
				// was actually a null
				if (object == LEGAL_NULL) object = null;
	...
	private void addAdapterMethod(final Method m) {
		...
		if (am.inheritable) inheritableAdaptables.put(am.adaptedType, am);
	}
	...
	/**
	 * Check whether the adapter method can produce output of type to,
	 * OR whether (for each converter) the adapter method can produce an output
	 * for which the converter can produce an output of type to
	 * 
	 * @param am
	 * @param to
	 * @return true if the adapter method can (with possible conversion) make a to
	 */
	...
	/**
	 * Using the given method, try and adapt from to an instanceof to; this may use a converter from
	 * {@link #delegates}.
	 * 
	 * @param am
	 * @param from
        ...
        try {
            ...
            for (final IConverter c : delegates) {
                if (c.adapts(adapterOutput, to)) {
                    ...
                    if (adapted != null) return cache(from, adapted, scope, am.shouldBeCached());
                }
            }
	...
	/**
	 * Turn from into a to in the scope. First we try all the exact adapters we know.
	 * Then we try all the inheritable adapters.
	 */
	@Override
	 ...
	 * (a) we can exactly adapt its type or (b) we can inheritably adapt its type
	 */
	@Override
