...
var drawgrid = function() {
    var total_weight = function(data) {
        return d3.sum(data.map(function(d) {return d.weight || 1.0;}));
    };
    ...
    var hist_plot = function(element, data, x) {
        ...
        bar.append("text")
            ...
            .text(function(d) { return formatCount(total_weight(d)); });

        bar.append("rect")
            .attr("x", 1)
            .attr("width", xscale(bins[0].x + bins[0].dx) - 2)
            .attr("height", function(d) { return inner_height - yscale(total_weight(d)); });
    ...
    var hexbin_plot = function(element, data, y, x) {
        ...
        if (data.length <= 600) {
            svg.selectAll("circle")
                ...
                .append("circle")
                .attr("r", "2px")
                .attr("cx", function(d) {return xscale(fx(d));})
                .attr("class", function(d) {return "_point_" + d.__counter;})
                .attr("cy", function(d) {return yscale(fy(d));})
            ...
            var points = data.map(function(datum) { return [xscale(fx(datum)), yscale(fy(datum))] ;});
            ...
            svg.append("g")
                ...
                .style("fill", function(d) { return color(total_weight(d)); });
        }
    };
