...
** Appliances Base Energy
   ...
   This is a multiplier to the appliances formula. It is 23.705225 W in SAP and 21.081451 W BREDEM. The formula is otherwise the same.
...
** Thermal Bridging Coefficient
   ...
   Note that there is a minor error with the specification of units in BREDEM 2012. The units for the /thermal bridge factor/ (should be W/m^2/K) and the /thermal bridging loss/ (should be W/K) are reversed.
...
** Responsiveness
   ...
   Removed the /of:/ property from =house.heating-responsiveness=, since only the responsiveness of the primary space heating system has an effect.
   ...
   It is possible to set the responsiveness on =measure.storage-heating=. This is to allow for the modelling of improved modern storage heating. In SAP 2012 mode, any changes to storage heating responsiveness made in the scenario are ignored.
...
** Storage Combi Storage Losses
   ...
   In the stock import, this value is set based on /pcdbmatch/ from
   /space-heating.csv/. This means that it will be /ManufacturerDeclared/ if we
   found a match for it in the PCDB.
...
** Thermal Mass
   ...
   Internal and Party walls are not included in this calculation. Nor are roofs or floors.
...
*** Door Glazing
    ...
    Doors are double glazed if the proportion of double glazing in the dwelling is > 0.5, and single glazed otherwise. (This method was chosen since a large number of dwellings have only 1 door, in which case it is a good approximation.)
...
** House Facing
   ...
   There was also a bug in the previous version of the NHM which made all dwellings face /North-East/, so dwellings are now getting more sunlight on average.
...
** Heating Schedule
   ...
   In BREDEM 2012 mode, can set heating schedules using =action.set-heating-schedule=.
...
** Weather
   ...
   The energy calculator in the NHM doesn't know whether or not you are using the energy consumption to compute a SAP score, so we've chosen to always use the UK average weather (and ignore user-specified weather) when you are in SAP 2012 mode.

   In BREDEM 2012 mode, the user-specified weather will always be used.
...
*** SAP Window U-Values
    ...
    We always use the most pessimistic value for the /glazing gap/ (6mm) and /εn value/, as we have no information about these.
...
**** Party Walls
    ...
    In SAP 2012, party walls with unfilled cavities have a u-value of 0.5. This is based on Tables 3.6 and S8B. It simulates the movement of air within a cavity wall.

    We have assumed that /Metal Frame/ and /Cob/ party walls count as not cavity for this purpose.
