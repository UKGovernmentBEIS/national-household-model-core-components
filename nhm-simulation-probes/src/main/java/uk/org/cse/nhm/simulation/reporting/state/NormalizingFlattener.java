...
/**
 * An {@link IComponentFlattener} which serializes out the structure state into mongodb.
 *
...
public abstract class NormalizingFlattener<T> implements IComponentFlattener {
	...
	/**
	 * Convert a dwelling's structure into an ID, and if necessary store the ID <-> house case mapping into mongo.
	 */
	@Override
	...
	/**
	 * Yield a new ID for the house case, or an existing ID if the same house case has already passed through
	 * 
	 * this is not synchronized, so it is dangerous. Synchronizing it makes it extraordinarily slow.
	 * @param hc
	 * @return an ID
	 */
